...
import { Button, Input, Select, Card, CardHeader, CardTitle, CardContent, Badge, Modal, Alert, CredentialsModal } from '../../components/common';
...
import { Search, Plus, Users, GraduationCap, Eye, Edit, Trash2, School, BookOpen, Mail } from 'lucide-react';
...
const generateCredentials = (email: string) => {
  ...
  const password = `${SCHOOL_CODE}@202
  const { canManageClasses } = useRole();4`;
...
const ClassList: React.FC = () => {
  ...
  const handleAddClass = () => {
    // Check if form teacher needs credentials first
    const newFormTeacherId = newClass.formTeacherId ? parseInt(newClass.formTeacherId) : null;
    
    if (selectedClass) {
      // Editing existing class
      const previousFormTeacher = getFormTeacher(selectedClass.id);
      const previousFormTeacherId = previousFormTeacher?.id;
      
      if (newFormTeacherId && newFormTeacherId !== previousFormTeacherId) {
        // Form teacher was assigned or changed - show credentials first
        const teacher = mockTeachers.find(t => t.id === newFormTeacherId);
        if (teacher) {
          const credentials = generateCredentials(teacher.email);
          setGeneratedCredentials({ 
            ...credentials, 
            teacherEmail: teacher.email,
            teacherName: getFullName(teacher.first_name, teacher.last_name)
          });
          setShowAddModal(false);
          setShowCredentialsModal(true);
          return; // Show credentials modal first
        }
      }
      
      // No credentials needed, just save
      ...
      // No form teacher selected, just save
  ...
  const handleCredentialsModalClose = () => {
    // Complete the class save operation
    if (selectedClass) {
      console.log('Class updated successfully with form teacher');
    } else {
      console.log('Class created successfully with form teacher');
    }
    ...
    <div className="space-y-6">
      ...
      >
        <div className="space-y-4">
          ...
          {newClass.formTeacherId && (() => {
            const selectedTeacher = mockTeachers.find(t => t.id === parseInt(newClass.formTeacherId));
              ...
              return (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <div className="flex items-start gap-3">
                    ...
                    />
                    <div className="flex-1">
                      <label htmlFor="sendEmailClass" className="text-sm font-medium text-blue-900 cursor-pointer">
                        Send credentials via email
                      </label>
                      <p className="text-xs text-blue-700 mt-1">
                        <Mail className="h-3 w-3 inline mr-1" />
                        Login credentials will be automatically generated and sent to <strong>{selectedTeacher.email}</strong>
                      </p>
                    </div>
                  </div>
                </div>
